...
def update_storage_node(id):
    ...
    try:
        updated_node = storage_node_schema.load(data, instance=node, partial=False)
        db.session.commit()
        return storage_node_schema.dump(updated_node), 200
    except ValidationError as err:
        return jsonify(err.messages), 400
    except Exception as e:
        db.session.rollback()
...
@storage_nodes_bp.route("/<int:id>", methods=["PATCH"])
def patch_storage_node(id):
    node = StorageNode.query.get(id)
    if not node:
        return jsonify({"error": "StorageNode not found"}), 404
